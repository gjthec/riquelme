...
const Step = ({ icon, text, subtext }: { icon: React.ReactNode, text: string, subtext?: string }) => (
  <div className="flex flex-col items-center text-center space-y-8 w-full md:flex-1 group">
    ...
    <div className="space-y-3 max-w-[150px] md:max-w-none px-2">
      <p className="text-[10px] md:text-[11px] font-light uppercase tracking-[0.15em] leading-tight text-brand-offwhite group-hover:text-brand-cream transition-colors">
        {text}
      </p>
      {subtext && (
        <p className="text-[9px] text-brand-taupe uppercase leading-tight font-thin tracking-[0.2em]">
          {subtext}
        </p>
      )}
    </div>
  ...
  return (
    <section className="py-32 bg-brand-dark border-y border-white/5">
      <div className="text-center mb-28">
        <h2 className="text-3xl md:text-5xl font-serif italic uppercase tracking-[0.25em] text-brand-cream font-extralight">
          Metodologia
        </h2>
      ...
      <div className="flex flex-col md:flex-row justify-between items-center md:items-start gap-2 max-w-6xl mx-auto px-6">
        ...
        <Step 
          ...
          icon={
            <svg fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={0.8} d="M8 12h.01M12 12h.01M16 12h.01M21 12c0 4.418-4.03 8-9 8a9.863 9.863 0 01-4.255-.949L3 20l1.395-3.72C3.512 15.042 3 13.574 3 12c0-4.418 4.03-8 9-8s9 3.582 9 8z" />
            </svg>
          } 
        />
        <Arrow />
        <Step 
          ...
          icon={
            <svg fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={0.8} d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" />
            </svg>
          } 
        />
      </div>
